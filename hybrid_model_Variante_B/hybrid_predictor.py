"""
Hybrid Predictor Class für Streamlit Deployment (Variante B mit Professional Titles)
"""
...
class HybridPredictor:
    """
    Hybrid Model: TF-IDF + Random Forest + Professional Titles List
    """

    def __init__(self, model_dir='hybrid_model_Variante_B'):
        ...
        # Load Professional Titles
        with open(f'{model_dir}/professional_titles.pkl', 'rb') as f:
            self.professional_titles = pickle.load(f)
    ...
    def predict_seniority(self, person_jobs: List[Dict], target_job_idx: int = 0) -> Dict[str, Any]:
        ...
        if text in self.professional_titles:
            return {
                'label': 'Professional',
                'confidence': 1.0,
                'source': 'professional_list'
            }
